		...
		<view class="video-list song-list">
			<view class="video-item" v-for="(item, index) in list" :key="index">
				<view class="video-title-bar">
					<img class="videoImg" src="" alt="头像">
					<view class="video-title-bar-name">
						{{item.creator[0].userName}}
					</view>
				</view>
				<img class="img" :src="item.coverUrl" alt="图片">
	...
	.video-list{
		...
		.video-item{
			...
			.video-title-bar {
				...
				.video-title-bar-name {
					width: 80%;
				}
			}
		}
		.img {
			// width: var(--videoImgW, 686rpx );
			// height: var(--videoImgH, 390rpx);
			display: block;
			width: 686rpx;
			height: 390rpx;
			background: #eee;
