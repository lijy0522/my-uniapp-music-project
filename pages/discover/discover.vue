	...
	<view>
		...
		<view class="banner">
			<swiper 
				...
				:duration="1000"
			>
				<swiper-item v-for="(item, index) in _swipers" :key="index">
					<view class="item">
		...
		<view class="main-bar flex-box">
			<view class="flex-item" v-for="(item, index) in contentBar" :key="index">
				<img :src="'../../static/image/index/t_' + (index + 1) + '.png'" alt="入口图标" class="img">
				<view>{{item.name}}</view>
			</view>
		</view>
		<!-- 推荐歌单 -->
		...
		methods:{
			...
			getRecommendedSongs(){
				...
				reqGetRecommendedSongs(params).then(res => {
					...
					//格式化
					this.recommendedSongs.forEach(item => {
