<!-- 播客页面 -->
<template>
	<view>
		<uni-section type="line" style="margin-bottom: 3px;">
			<view class="box-bg">
				<uni-nav-bar>
					<view class="input-view">
						<uni-icons class="input-uni-icon" type="search" size="18" color="#999" />
						<input confirm-type="search" class="nav-bar-input" type="text" placeholder="搜索关键词"
							@confirm="confirm" />
					</view>
				</uni-nav-bar>
			</view>
		</uni-section>
	</view>
	
	<view>
		<!-- 猜你喜欢 -->
		<ScrollViewX title="猜你喜欢" link="" :list="youLike" ></ScrollViewX>
		<!-- 真实故事 -->
		<ScrollViewX title="真实故事" link="" :list="realStory" ></ScrollViewX>
		<!-- 情感 -->
		<ScrollViewY title="情感" :list="emotion" link="" ></ScrollViewY>
		<!-- 知识 -->
		<ScrollViewX title="知识" link="" :list="knowledges" ></ScrollViewX>
	</view>
</template>

<script>
	import {reqGetRecommendedSongs} from '@/api/index.js'
	import ScrollViewX from '@/components/ScrollViewX.vue'
	import ScrollViewY from '@/components/ScrollViewY.vue'
	export default {
		data() {
			return {
				youLike: [],		// 猜你喜欢
				realStory: [], 		// 真实故事
				emotion: [],		// 情感
				knowledges: [],		// 知识
			};
		},
		components:{
			ScrollViewX,
			ScrollViewY
		},
		onLoad() {
			this.getYouLike()
			this.getRealStory()
			this.getEmotion()
			this.getKnowledges()
		},
		methods: {
			getYouLike() {
				const params = {
					limit: 6
				}
				reqGetRecommendedSongs(params).then(res => {
					this.youLike = res.result
				})
			},
			getRealStory() {
				const params = {
					limit: 3
				}
				reqGetRecommendedSongs(params).then(res => {
					this.realStory = res.result
				})
			},
			getEmotion() {
				const params = {
					limit: 3
				}
				reqGetRecommendedSongs(params).then(res => {
					this.emotion = res.result
				})
			},
			getKnowledges() {
				const params = {
					limit: 3
				}
				reqGetRecommendedSongs(params).then(res => {
					this.knowledges = res.result
				})
			}
		}
	}
</script>

<style lang="scss" scoped>
	$nav-height: 30px;
	.box-bg {
		background-color: #F5F5F5;
		padding: 5px 0;
		
		.input-view {
			/* #ifndef APP-PLUS-NVUE */
			display: flex;
			/* #endif */
			flex-direction: row;
			// width: 500rpx;
			flex: 1;
			background-color: #f8f8f8;
			height: $nav-height;
			border-radius: 15px;
			padding: 0 15px;
			flex-wrap: nowrap;
			margin: 7px 0;
			line-height: $nav-height;
			
			.input-uni-icon {
				line-height: $nav-height;
			}
		}
		
	}
	
	
	
	.song-list-comp {
		padding-left: 32rpx;
		margin-top:10rpx;
		.tit-bar {
			font-size: 34rpx;
			font-weight: 600;
			line-height: 110rpx;
		}

		.more {
			width:150rpx;
			height:50rpx;
			line-height:50rpx;
			text-align: center;
			margin-top: 30rpx;
			margin-right: 32rpx;
			font-weight: 400;
			font-size: 24rpx;
			border: 2rpx solid #e6e6e6;
			border-radius: 50rpx;
		}

		.item {
			display: flex;
			position: relative;
			width: 100%;
			height: 5rem;
			padding-bottom:16rpx;
			margin-right: 16rpx;
			line-height: 34rpx;
			&.video{
				width: 450rpx;
			}
			.img {
				display: block;
				width: 20%;
				height: 4.5rem;
				margin-bottom: 16rpx;
				background: #eee;
				border-radius: 10rpx;
			}
			.desc{
				height:64rpx;
				margin:1rem .5rem .3rem;
				line-height: 30rpx;
				overflow: hidden;
			}
			.count{
				position:absolute;
				top:0;
				right:0;
				padding-left:25rpx;
				line-height: 34rpx;
				color:#fff;
				font-size: 24rpx;
				z-index: 10;
				background: url(data:image/png;base64,) no-repeat;
				background-size: 25rpx 25rpx;
				transform: scale(0.8);
			}
		}
	}
</style>
